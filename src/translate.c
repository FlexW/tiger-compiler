               ...
               util_bool_list *formals_ptr)
{
  ...
  *static_link         = true;
  ...
  if (formals_ptr == NULL)
    formals_ptr = linked_list_new ();
  linked_list_insert ((linked_list*)formals_ptr, 0, static_link);
 ...
 */
tra_exp *
tra_array_exp (tra_exp *size_ptr,
               tra_exp *init_ptr)
{
  /* Call external function that handels this */
  tree_exp_list *args = linked_list_new ();
  linked_list_add (args, conv_exp (size_ptr));
  linked_list_add (args, conv_exp (init_ptr));
 ...
 */
tra_exp *
tra_record_exp (tra_exp_list *list_ptr)
{
  /*
  ...
  */
  ...
  int       s         = linked_list_length (list_ptr) * frm_word_size;
  tree_exp *size      = tree_new_const (s);
  tree_exp_list *args = linked_list_new ();
  linked_list_add (args, size);
  ...
  tra_exp *exp;
  LINKED_LIST_FOR_EACH (exp, list_ptr)
    {
      tree_exp *addr  = tree_new_bin_op (TREE_PLUS,
                                        addr,
                                        tree_new_const (i * frm_word_size));
      tree_stm *init2 = tree_new_move (addr, conv_exp (exp));
               ...
               temp_label *done_ptr)
{
  ...
  temp_label_list *label_list = linked_list_new ();
  linked_list_add (label_list, test_label);
...
{
  temp_label_list *label_list = linked_list_new ();
  linked_list_add (label_list, done_ptr);
 ...
 */
tra_exp *
tra_seq_exp (tra_exp_list *list_ptr)
{
  tree_stm *tree_seq_start = NULL;
  tree_stm *tree_seq       = NULL;

  if (linked_list_length (list_ptr) == 1)
    return linked_list_get (list_ptr, 0);

  tra_exp *tra_exp;
  LINKED_LIST_FOR_EACH (tra_exp, list_ptr)
    {
      tree_stm *exp = conv_no_res_exp (tra_exp);

      if (tree_seq == NULL)
        {
          tree_seq = tree_new_seq (exp, NULL);
          tree_seq_start = tree_seq;
        }
      else if (tree_seq->u.seq.right == NULL)
        {
          tree_seq->u.seq.right = exp;
        }
      else
        {
          tree_seq->u.seq.right = tree_new_seq (tree_seq->u.seq.right, exp);
          tree_seq = tree_seq->u.seq.right;
        }
    }
  assert (tree_seq->u.seq.right); /* Error if NULL */
  return trans_no_res_exp (tree_seq_start);
}
...
{
  tree_exp_list *tree_list = linked_list_new ();
  tree_exp      *fun       = tree_new_name (fun_ptr);

  tra_exp *tra_exp;
  LINKED_LIST_FOR_EACH (tra_exp, exp_list_ptr)
    {
      tree_exp *exp = conv_exp (tra_exp);
      linked_list_add (tree_list, exp);
    }
  /* Add static link */
  linked_list_add (tree_list, get_offset (fun_call_level_ptr,
                                          fun_dec_level_ptr,
                                          frm_frame_pointer ()));
  return trans_exp (tree_new_call (fun, tree_list));
}
            ...
            temp_temp *frame_pointer_ptr)
{
  ...
  tra_access *access          = linked_list_get (used_level_ptr->formals, 0);
  ...
  while (used_level_ptr != declared_level_ptr
         && used_level_ptr != outermost_level)
    {
      ...
      access         = linked_list_get (used_level_ptr->formals, 0);
 ...
 */
static void
frag_list_add (frm_frag *frag_ptr)
{
  if (frag_list == NULL)
      frag_list = linked_list_new ();

  linked_list_add (frag_list, frag_ptr);
}
...
{
  tra_access_list *tra_access_list = linked_list_new ();
  frm_access_list *frm_access_list = frm_formals (level->frame);

  if (frm_access_list == NULL)
    assert (0); /* TODO: fix this */

  frm_access *frm_access;
  LINKED_LIST_FOR_EACH (frm_access, frm_access_list)
    {
      tra_access *access = new (sizeof (*access));
      access->access     = frm_access;
      access->level      = level;

      linked_list_add (tra_access_list, access);
    }

  return tra_access_list;
}
